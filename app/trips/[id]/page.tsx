...
type Invoice = {
  ...
  file_url: string | null; file_path: string | null; name?: string | null; uploaded_at?: string | null;
  amount?: number | null; currency?: string | null;
  flight_id?: string | null; accommodation_id?: string | null; transport_id?: string | null
}
...
export default function TripDetailPage() {
  ...
  // trip state BEFORE deriving permissions
  ...
  // quick create forms: accommodation (✅ added cost + currency)
  ...
  const [accCost, setAccCost] = useState('');                    // ✅ NEW
  const [accCurrency, setAccCurrency] = useState('USD');         // ✅ NEW
  const [accInvoice, setAccInvoice] = useState<File | null>(null)

  // quick create forms: transport (✅ added currency)
  ...
  const [tCurrency, setTCurrency] = useState('USD')              // ✅ NEW
  ...
  async function reloadTrip() {
    ...
    const fl = await sb.from('flights').select('*').eq('trip_id', id)
    if (!fl.error) setFlights((fl.data || []) as Flight[])

    const ac = await sb.from('accommodations').select('*').eq('trip_id', id)
    if (!ac.error) setAccs((ac.data || []) as Accommodation[])

    const tr = await sb.from('transports').select('*').eq('trip_id', id)
    if (!tr.error) setTrans((tr.data || []) as Transport[])

    const inv = await sb.from('invoices').select('*').eq('trip_id', id).order('uploaded_at', { ascending: false })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!user) { if (!cancel) setStatus('need-login'); return }
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    const { error } = await sb
                      ...
                      .update({
                        status: 'approved',
                        submitted_at: trip?.submitted_at ?? new Date().toISOString(),
                        approved_by: me!.id,
          ...
          <div className="row-left">
            <IconHotel /><div style={{ minWidth: 0 }}>
              ...
              <div className="row-sub">
                {a.address || '—'} • {a.check_in || '—'} → {a.check_out || '—'} • Cost: {money(a.cost, a.currency)}
              </div>
            </div>
          </div>
          <div className="row-right">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 8 }}>
              ...
              <label className="block"><span className="label">Cost</span>
                <input className="input" type="number" step="0.01"
                       defaultValue={a.cost ?? ''}
                       onChange={(e) => a.cost = e.target.value ? Number(e.target.value) : null} />
              </label>
              <label className="block"><span className="label">Currency</span>
                <select className="input" defaultValue={a.currency || 'USD'}
                        onChange={(e) => a.currency = e.target.value || null}>
        ...
        <div className="row" aria-expanded={open} onClick={() => setOpenTrans(s => ({ ...s, [t.id]: !open }))}>
          <div className="row-left">
            <IconCar /><div style={{ minWidth: 0 }}>
              ...
              <div className="row-sub">
                {(t.pickup_location || '—')} → {(t.dropoff_location || '—')} • {fmtRange(t.start_time, t.end_time)} • Cost: {money(t.cost, t.currency)}
              </div>
            </div>
          </div>
          <div className="row-right">
        ...
        {open && (
          <div className="details">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 8 }}>
              ...
              {/* ✅ NEW: Transport Currency */}
              <label className="block"><span className="label">Currency</span>
                <select className="input" defaultValue={t.currency || 'USD'} onChange={(e) => t.currency = e.target.value || null}>
                  <option>USD</option><option>EUR</option><option>GBP</option><option>ILS</option>
                  <option>AUD</option><option>CAD</option>
                </select>
              </label>
              ...
              {showAccForm && (
                <div className="details" onClick={e => e.stopPropagation()}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 8 }}>
                    ...
                    {/* ✅ NEW: Cost + Currency in Add form */}
                    <label className="block"><span className="label">Cost</span>
                      <input className="input" type="number" step="0.01" value={accCost} onChange={(e) => setAccCost(e.target.value)} placeholder="e.g. 650.00" />
                    </label>
                    <label className="block"><span className="label">Currency</span>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <button className="btn-primary" onClick={async () => {
                      const ins = await sb.from('accommodations').insert({
                        trip_id: id,
                        name: accName, address: accAddr,
                        check_in: accIn || null, check_out: accOut || null,
                        booking_ref: accRef || null,
                        cost: accCost ? Number(accCost) : null,          // ✅ include
                        currency: accCurrency || null                    // ✅ include
                      }).select().single()
                      if (ins.error) return alert(ins.error.message)
                      if (ins.data?.id && accInvoice) {
                        try { await uploadInvoice('accommodation', ins.data.id, accInvoice) } catch (e: any) { alert('Accommodation saved, invoice upload failed: ' + (e?.message || 'unknown')) }
                      }
                      setAccName(''); setAccAddr(''); setAccIn(''); setAccOut(''); setAccRef(''); setAccInvoice(null)
                      setAccCost(''); setAccCurrency('USD')            // ✅ reset
              ...
              {showTransForm && (
                <div className="details" onClick={e => e.stopPropagation()}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 8 }}>
                    ...
                    {/* ✅ NEW: Currency selector in Add form */}
                    <label className="block"><span className="label">Currency</span>
                      <select className="input" value={tCurrency} onChange={(e) => setTCurrency(e.target.value)}>
                        <option>USD</option><option>EUR</option><option>GBP</option><option>ILS</option>
                        <option>AUD</option><option>CAD</option>
                      </select>
                    </label>
                  ...
                  <div style={{ marginTop: 8 }}>
                    <button className="btn-primary" onClick={async () => {
                      const ins = await sb.from('transports').insert({
                        ...
                        currency: tCurrency || null                              // ✅ include
                      }).select().single()
                      if (ins.error) return alert(ins.error.message)
                      ...
                      setTCompany(''); setTFrom(''); setTTo(''); setTStart(''); setTEnd(''); setTCost(''); setTCurrency('USD'); setTInvoice(null)
