...
const sb = useMemo(() => createClient(), []) {
  ...
  async function sendToAI() {
    try {
      ...
      // include a bearer for bullet-proof auth on Netlify
      const { data: { session } } = await sb.auth.getSession()
      const token = session?.access_token
    ...
    <div className="space-y-6">
      <div className="trip-cover" style={{ padding: 16, borderRadius: 16 }}>
        ...
        <div className="row-sub">
          Attach receipts/itineraries or describe the trip. The AI will draft flights, accommodation, transport and itinerary
          events.
        </div>
      </div>
